...
def run_sanity_check(test_dir):
    ...
    # assert path.isdir(test_dir), FAIL_COLOR+f"No
    # direcotry named {test_dir} found in {os.getcwd()}"
    print(
        "This script will perform a sanity test to \
            ensure your code meets the criteria in the rubric.\n"
    )
    print(
        "Please enter the path to the file that \
            contains your test cases for the GET() and POST() methods"
    )
    ...
    test_function_names = list(
        filter(
            lambda x: inspect.isfunction(
                getattr(module, x)) and not x.startswith("__"),
            dir(module),
        )
    )
    ...
    if not test_functions_for_get:
        ...
        if not TEST_FOR_GET_METHOD_RESPONSE_CODE:
            ...
            print(
                FAIL_COLOR
                + "Your test case for GET() does not \
    ...
    if not test_functions_for_post:
        print(FAIL_COLOR + f"[{WARNING_COUNT}]")
        WARNING_COUNT += 1
        print(FAIL_COLOR + "No test cases were\
              detected for the POST() method.")
        print(
            FAIL_COLOR
    ...
    print(
        WARN_COLOR
        + "This is a heuristic based sanity \
            testing and cannot guarantee the correctness of your code."
    )
    print(
        WARN_COLOR
        + "You should still check your work against \
            the rubric to ensure you meet the criteria."
    )
